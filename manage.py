...
def drive(cfg, use_joystick=False, camera_type='single'):
    ...
    temp = input("Please insure the car is at or near the origin. Input yes or y to continue: ")
    if temp == "y" or temp == "yes" or temp == "Y" or temp == "YES":
        origin_reset.reset_origin()
    print("The origin is reset. Let's get it started!\n")
    ...
    # Give car 10 seconds to move, check the coordinate after that
    time.sleep(10)
    #if run.pos.x < 1 and run.pos.x > -1 and run.pos.y < 1 and run.pos.y > -1:
        #return
...
if __name__ == '__main__':
    ...
    if args['drive']:
        while 1:
            # Run Rizzi's code for text detection
            # It shall return the decoded_text we want
            decoded_text = mainFunc_cam();
            #print(detected_text)
            
            # No need to worry about other condition here
            # If text is not what we want, mainFunc_cam() will not terminate
            # It's already done in camera.py
            if decoded_text == "001":
                cfg.PATH_FILENAME = "path1.csv"
            elif decoded_text == "002":
                cfg.PATH_FILENAME = "path2.csv"
            elif decoded_text == "003":
                cfg.PATH_FILENAME = "path3.csv"
            elif decoded_text == "004":
                cfg.PATH_FILENAME = "path4.csv"
            
            drive(cfg, use_joystick=args['--js'], camera_type=args['--camera'])
        
        """
        Used to test config change, it works
        print("Current PATH_FILENAME is: " + cfg.PATH_FILENAME)
        # x here can be the detected decoded_text
        x = input("What's the digit detected?\n")
        while x != "3":
            if x == "1":
                cfg.PATH_FILENAME = "path1.csv"
                # drive(cfg, use_joystick=args['--js'], camera_type=args['--camera'])
                ...
                cfg.PATH_FILENAME = "path2.csv"
            print(cfg.PATH_FILENAME)
            x = input("\nWhat's the new digit detected")
        """
